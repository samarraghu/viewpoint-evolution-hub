...
export function Filters() {
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2 px-6 py-2 bg-card border-b">
      {slicers.map((s) => {
        const opts = s.label === "Brand" ? brandOptions : s.label === "Period" ? periodOptions : [s.value];
        return (
          <label key={s.label} className="flex flex-col gap-0.5">
            <span className="text-[10px] uppercase tracking-wide text-muted-foreground">{s.label}</span>
            <select
              defaultValue={s.value}
              className="bg-background border rounded-md px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-ring"
            >
              {opts.map((o) => (
